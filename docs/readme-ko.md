...
- 결제 이력을 초기화하지 않고 구독의 종료일/투숙객 이름/요금을 그 자리에서 수정
...
`show`, `update`, `pay`, `deactivate`는 `--room`, `--plate`, `--id` 중 정확히 하나를 받는다. 방 번호가 권장 식별자다 — 한 번 배정되면 고정불변(방 하나당 차량 하나)이라, `register`는 이미 활성 구독이 있는 방으로 재등록을 거부한다.

`update`는 전달한 필드(`--end-date`, `--guest-name`, `--monthly-fee`)만 바꾸고 결제 이력은 그대로 둔다 — 오탈자를 고치거나 구독을 연장할 때는 `deactivate` + `register` 대신 이걸 쓴다. `prune-outbox`는 보존 기간(기본 30일)보다 오래된, 이미 발송된 알림을 삭제한다. 아직 발송 안 된 알림은 건드리지 않는다.
